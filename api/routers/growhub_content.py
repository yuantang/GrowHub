...
@router.post("/generate_mock")
async def generate_mock_data(count: int = Query(20, ge=1, le=100)):
    """生成模拟数据(用于演示)"""
    import random
    from datetime import timedelta
    
    platforms = ["douyin", "xiaohongshu", "bilibili", "weibo", "zhihu"]
    sentiments = ["positive", "neutral", "negative"]
    categories = ["sentiment", "hotspot", "competitor", "influencer", "general"]
    
    # 模拟数据源
    titles = [
        "关于 MediaCrawler 的使用体验", "最近很火的 AI 工具测评", "竞品分析报告：GrowHub vs 竞品",
        "如何搭建高效的舆情监控系统", "Python 爬虫实战教程", "2026年社交媒体营销趋势",
        "遇到了一个严重的 Bug，求助", "强烈推荐！这款工具太好用了", "避雷！千万不要用这个功能",
        "MediaCrawler 源码解读", "GrowHub 新功能抢先看", "数据分析师的日常",
        "为什么你的爬虫总是被封？", "从零开始学习 Agent 开发", "企业级知识库搭建指南",
        "效率提升 10 倍的秘密", "不得不看的行业内幕", "深夜吐槽...", "今日份开心", "干货分享"
    ]
    ...
    async with get_session() as session:
        for _ in range(count):
            ...
            crawl_time = datetime.now() - timedelta(hours=random.randint(0, 168))
            publish_time = crawl_time - timedelta(hours=random.randint(1, 48))
            
            content = GrowHubContent(
                platform=platform,
                content_type=random.choice(["note", "video", "article"]),
                platform_content_id=f"mock_{random.randint(100000, 999999)}",
                title=random.choice(titles),
                description=f"这是一条生成的模拟内容描述，用于演示 GrowHub 的功能。平台：{platform}，情感：{sentiment}...",
                content_url="https://example.com/mock",
                ...
                like_count=int(view_count * (random.random() * 0.1)),
                comment_count=int(view_count * (random.random() * 0.01)),
                share_count=int(view_count * (random.random() * 0.005)),
                engagement_rate=random.random() * 0.2,
                
                # 分析结果
                sentiment=sentiment,
                sentiment_score=random.uniform(-1, 1),
                category=random.choice(categories),
                keywords=random.sample(["爬虫", "AI", "数据", "监控", "营销"], 2),
                
                # 预警
                is_alert=is_alert,
                alert_level=alert_level,
                alert_reason="模拟触发预警规则" if is_alert else None,
                is_handled=False
            )
            session.add(content)
            generated_count += 1
    ...
    return {"message": f"成功生成 {generated_count} 条模拟数据", "count": generated_count}
