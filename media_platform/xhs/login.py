...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_cookies(self):
        ...
        # IMPORTANT: Only inject web_session cookie!
        # Other cookies (a1, webId, etc.) should be generated by the browser itself
        # to match the current environment. Injecting them causes anti-bot detection!
        ...
        utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Injecting web_session cookie only (length: {len(web_session)})")
        ...
        utils.logger.info("[XiaoHongShuLogin.login_by_cookies] Successfully injected web_session cookie")
        
        # Reload the page to apply the new cookie
        try:
            current_url = self.context_page.url
            if current_url == "about:blank":
                current_url = "https://www.xiaohongshu.com/"
            
            utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Navigating to {current_url} to apply cookie ...")
            await self.context_page.goto(current_url, wait_until="networkidle")
            ...
            utils.logger.error(f"[XiaoHongShuLogin.login_by_cookies] Error reloading page: {e}")
