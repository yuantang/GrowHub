...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def login_by_cookies(self):
        ...
        if not cookie_dict:
            utils.logger.error("[XiaoHongShuLogin.login_by_cookies] Cookie string is empty or invalid!")
            return
        
        # Check for required cookies
        web_session = cookie_dict.get("web_session")
        if not web_session:
            utils.logger.error("[XiaoHongShuLogin.login_by_cookies] web_session not found in cookies!")
            return
        
        # Inject ALL cookies to maintain consistent environment
        # This is critical for signature generation - a1, webId, etc. must match
        cookies_to_inject = []
        ...
        utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Injecting {len(cookies_to_inject)} cookies")
        
        # Log which essential cookies are present
        present_essential = [c for c in essential_cookies if c in cookie_dict]
        utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Essential cookies found: {present_essential}")
        
        await self.browser_context.add_cookies(cookies_to_inject)
        utils.logger.info("[XiaoHongShuLogin.login_by_cookies] Successfully injected cookies")
        ...
        try:
            ...
            utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] Navigating to {current_url} to apply cookies ...")
            await self.context_page.goto(current_url, wait_until="domcontentloaded", timeout=30000)
            
            # Wait for XHS JS to fully load (critical for signature generation)
            utils.logger.info("[XiaoHongShuLogin.login_by_cookies] Waiting for XHS JS to load (window.mnsv2)...")
            max_wait = 15  # seconds
            for i in range(max_wait):
                try:
                    mnsv2_exists = await self.context_page.evaluate("() => typeof window.mnsv2 === 'function'")
                    if mnsv2_exists:
                        utils.logger.info(f"[XiaoHongShuLogin.login_by_cookies] window.mnsv2 loaded after {i+1} seconds")
                        break
                except Exception:
                    pass
                await asyncio.sleep(1)
            else:
                utils.logger.error("[XiaoHongShuLogin.login_by_cookies] window.mnsv2 NOT loaded after 15 seconds!")
            
            # Give extra time for localStorage initialization (b1 value)
            await asyncio.sleep(2)
            
            # Verify b1 is now in localStorage
            try:
                b1_exists = await self.context_page.evaluate("() => !!window.localStorage.getItem('b1')")
                if b1_exists:
                    utils.logger.info("[XiaoHongShuLogin.login_by_cookies] b1 found in localStorage ✓")
                else:
                    utils.logger.warning("[XiaoHongShuLogin.login_by_cookies] b1 not found in localStorage - signature may fail")
            except Exception as e:
                utils.logger.warning(f"[XiaoHongShuLogin.login_by_cookies] Could not check b1: {e}")
