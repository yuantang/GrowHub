...
const ImageGalleryModal: React.FC<{
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`图片预览 (${index + 1}/${images.length})`} className="max-w-5xl h-[85vh]">
                                ...
                                {images.map((img, idx) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            index === idx ? "border-white scale-110" : "border-transparent opacity-60 hover:opacity-100"
                                        )}
                                    >
...
// 2. Video Player Modal (Replicated from DataAnalysis.tsx)
...
export const ContentDataTable: React.FC<ContentDataTableProps> = ({
    data, loading, total, page, pageSize, onPageChange, onPageSizeChange, className, adaptiveHeight = false, pageScroll = false
}) => {
    // Media State
    ...
    return (
        <div className={cn("space-y-4", className, adaptiveHeight && "flex flex-col h-full space-y-0 gap-4")}>
            <div className={cn(
                ...
                <div className={cn(
                    ...
                    adaptiveHeight ? "flex-1 min-h-0" : (!pageScroll && "max-h-[calc(100vh-220px)]")
                )}>
                    <table className="w-full text-sm text-left">
                        <thead className="text-xs text-muted-foreground uppercase bg-muted border-b border-border sticky top-0 z-20 shadow-sm">
                            <tr>
                                <th className="px-4 py-3 font-medium w-[70px] sticky left-0 z-30 bg-muted shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">平台</th>
                                <th className="px-4 py-3 font-medium w-[180px] sticky left-[70px] z-30 bg-muted shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">作者</th>
                                    ...
                                    <tr key={item.id} className="border-b transition-colors hover:bg-muted/50 group">
                                        ...
                                        <td className="p-4 align-top">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed" title={item.content.desc}>
                                                    {item.content.desc}
                                                </p>
                                                {/* 标签 */}
                                                {item.content.tags && item.content.tags.length > 0 && (
                                                    <div className="flex flex-wrap gap-1 mt-1">
                                                        {item.content.tags.slice(0, 5).map((tag, idx) => (
                                                            <span key={idx} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400">
                                                                #{tag}
                                                            </span>
                                                        ))}
                                                    </div>
                                        ...
                                        <td className="p-4 align-middle">
                                            ...
                                            >
                                                {item.media.cover ? (
                                                    <img
                                                        src={`/api/proxy/image?url=${encodeURIComponent(item.media.cover)}`}
                                                        alt="cover"
                                                ...
                                                <div
                                                    ...
                                                    style={{ display: item.media.cover ? 'none' : 'flex' }}
                                                >
                                                ...
                                                {(item.media.type === 'video' || !!item.media.video_url) && (
                                                    <>
                                                ...
                                                {/* 图片：右上角显示图片数量 */}
                                                {item.media.type === 'image' && item.media.image_list && item.media.image_list.length > 1 && (
                                                    <div className="absolute top-1 right-1 bg-black/70 backdrop-blur-[2px] rounded px-1 py-0.5 flex items-center gap-0.5">
                                                        <ImageIcon className="w-3 h-3 text-white" />
                                                        <span className="text-[9px] text-white font-medium">{item.media.image_list.length}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </td>
