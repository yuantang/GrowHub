...
import {
    ...
    Heart, MessageCircle, Share2, Calendar, Users, AlertTriangle
} from 'lucide-react';
import {
...
export interface ContentListViewProps {
    ...
    // 默认筛选参数
    defaultFilters?: Partial<GrowHubContentFilters>;
    ...
    // 自定义统计卡片
    customStatsCards?: React.ReactNode;
...
export const ContentListView: React.FC<ContentListViewProps> = ({
    ...
    const [filters, setFilters] = useState<GrowHubContentFilters>({
        page: 1,
        page_size: 20,
        sort_by: defaultFilters.sort_by || 'crawl_time',
        sort_order: defaultFilters.sort_order || 'desc',
        start_date: defaultDateRange.start,
        end_date: defaultDateRange.end,
        ...defaultFilters,
    });

    // Temp filter state for inputs
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (showStatsCards) {
                promises.push(fetchGrowHubStats(filters));
            }
            if (showCharts) {
                promises.push(fetchGrowHubTrend(7, filters));
                promises.push(fetchTopAnalysis(10, filters));
            }

            const results = await Promise.all(promises);
            
            setContents(results[0].items);
            setTotal(results[0].total);
            
            if (showStatsCards && results[1]) {
                setStats(results[1]);
            }
            if (showCharts) {
                if (results[showStatsCards ? 2 : 1]) setTrendData(results[showStatsCards ? 2 : 1].data || []);
                if (results[showStatsCards ? 3 : 2]) setTopData(results[showStatsCards ? 3 : 2] || []);
            }
        } catch (error) {
    ...
    }, [filters, showStatsCards, showCharts]);
