    ...
    return (
        <button
            ...
            className={cn(
                ...
                isActive ? "bg-white text-foreground shadow-sm" : "hover:bg-background/50 hover:text-foreground",
                className
            )}
            onClick={(e) => {
...
const TabsContext = React.createContext<{ value: string | undefined; setValue: (v: string) => void } | null>(null)
            ...
            {React.Children.map(children, child => {
                if (React.isValidElement(child)) {
                    return React.cloneElement(child, {
                        // @ts-ignore
                        activeValue: undefined, setActiveValue: undefined
                    })
                }
                return child
            })}
...
>(({ className, value, onClick, ...props }, ref) => {
    ...
    return (
        <button
            ...
            className={cn(
                "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
                isActive ? "bg-white text-foreground shadow-sm text-black" : "text-gray-500 hover:text-gray-900",
                className
            )}
            onClick={(e) => {
...
export { TabsRoot as Tabs, TabsListWithContext as TabsList, TabsTriggerWithContext as TabsTrigger, TabsContentWithContext as TabsContent }
