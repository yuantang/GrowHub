                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    ...
                    <Card className="bg-card/50 backdrop-blur">
                        <CardContent className="p-4">
                            ...
                            <div className="text-2xl font-bold text-yellow-400">{stats.by_status?.contacted || 0}</div>
                        </CardContent>
                    </Card>
                    <Card className="bg-card/50 backdrop-blur">
                        <CardContent className="p-4">
                            <div className="text-sm text-muted-foreground">合作中</div>
                            <div className="text-2xl font-bold text-green-400">{stats.by_status?.cooperating || 0}</div>
                        </CardContent>
                    </Card>
                </div>
            )}
            ...
            {creators.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {creators.map((creator) => {
                        const platform = PLATFORM_MAP[creator.platform] || { label: creator.platform, icon: '📌', color: 'bg-gray-500/20 text-gray-400' };
                        const status = STATUS_MAP[creator.status] || STATUS_MAP.new;
                        
                        return (
                            <Card key={creator.id} className="bg-card/50 backdrop-blur hover:shadow-lg transition-shadow">
                                <CardContent className="p-4">
                                    {/* Header */}
                                    <div className="flex items-start gap-3 mb-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <h3 className="font-medium truncate">{creator.author_name || '未知博主'}</h3>
                                            {creator.signature && (
                                                <p className="text-xs text-muted-foreground line-clamp-1">{creator.signature}</p>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 mb-3 text-center">
                                        <div className="bg-muted/30 rounded-lg p-2">
                                            <div className="text-lg font-bold">{formatNumber(creator.fans_count)}</div>
                                            <div className="text-xs text-muted-foreground">粉丝</div>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-2">
                                            <div className="text-lg font-bold">{formatNumber(creator.likes_count)}</div>
                                            <div className="text-xs text-muted-foreground">获赞</div>
                                        </div>
                                        <div className="bg-muted/30 rounded-lg p-2">
                                            <div className="text-lg font-bold">{creator.content_count}</div>
                                    ...
                                    <div className="flex items-center gap-2 pt-2 border-t border-border">
                                        ...
                                        {creator.author_url && (
                                            <Button
                                                ...
                                                size="sm"
                                                className="p-1"
                                                onClick={() => window.open(creator.author_url, '_blank')}
                                            >
