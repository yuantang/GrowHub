...
const ProjectDetailPage: React.FC = () => {
  ...
  const [statsChart, setStatsChart] =
    useState<ProjectStatsChartResponse | null>(null);
  ...
  const loadContents = async () => {
    try {
      ...
      const res = await fetchProjectContents(projectId, contentsPage, 20, {
        deduplicate_authors: deduplicateAuthors,
      });
    ...
    try {
      ...
      const payload = {
        ...
        keywords: keywordsStr
          ...
          .map((k) => k.trim())
          .filter(Boolean),
        sentiment_keywords: sentimentKeywordsStr
          .split(/[,，\n\s]+/)
          .map((k) => k.trim())
          .filter(Boolean),
        ...
        platforms: Array.from(
          new Set(
            (editForm.platforms || []).map((p) => platformNormalize[p] || p),
          ),
        ),
        // 自动设置预警标记
        alert_on_negative:
          editForm.purpose === "sentiment" &&
          (editForm.alert_channels?.length || 0) > 0,
        alert_on_hotspot:
          editForm.purpose === "hotspot" &&
          (editForm.alert_channels?.length || 0) > 0,
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1 flex items-center gap-3">
              ...
              {project.sentiment_keywords &&
                project.sentiment_keywords.length > 0 && (
                  <span className="flex items-center gap-1 text-amber-500/80">
                    <AlertTriangle className="w-3.5 h-3.5" />{" "}
                    {(project.sentiment_keywords || []).join(", ")}
                  </span>
                )}
              ...
              <span>
                ...
                {Array.from(
                  new Set(
                    project.platforms.map((p) => PLATFORM_MAP[p]?.label || p),
                  ),
                ).join(", ")}
              </span>
            </p>
        ...
        <TabsContent value="dashboard" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {project.total_crawled.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  今日 +{project.today_crawled}
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-rose-600">
                  {project.total_alerts.toLocaleString()}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  今日 +{project.today_alerts}
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  下次:{" "}
                  {project.next_run_at
                    ? new Date(project.next_run_at).toLocaleTimeString()
                    : "-"}
                </p>
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent>
                <div className="text-lg font-bold truncate">
                  {project.crawler_type === "search"
                    ? "综合搜索"
                    : project.crawler_type === "detail"
                      ? "详情抓取"
                      : "博主主页"}
                </div>
                <p className="text-xs text-muted-foreground mt-1 line-clamp-1">
                  ...
                  {project.sentiment_keywords &&
                    project.sentiment_keywords.length > 0 &&
                    ` | 舆情: ${project.sentiment_keywords.join(", ")}`}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  监控:{" "}
                  {Array.from(
                  ...
                  | 限量: {project.crawl_limit}条 | 范围:{" "}
                  {project.crawl_date_range || "不限"}天
                </p>
              </CardContent>
            </Card>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      className={cn(
                        ...
                        project.latest_checkpoint.status === "running"
                          ? "bg-blue-100 text-blue-700 animate-pulse"
                          : project.latest_checkpoint.status === "completed"
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      <div className="text-2xl font-bold text-primary">
                        {project.latest_checkpoint.total_notes}{" "}
                        <span className="text-xs font-normal text-muted-foreground">
                          条内容
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        页码: {project.latest_checkpoint.current_page} | 评论:{" "}
                        {project.latest_checkpoint.total_comments}
                      </p>
                    </div>
                    {project.latest_checkpoint.total_errors > 0 && (
                      <div className="text-xs text-rose-500 font-medium flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" />{" "}
                        {project.latest_checkpoint.total_errors} 错误
                      </div>
                    )}
                  </div>
                </CardContent>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <Card className="col-span-2">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <TrendingUp className="h-4 w-4" /> 7日情感趋势
                </CardTitle>
              </CardHeader>
            ...
            <Card className="col-span-1">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  <PieChartIcon className="h-4 w-4" /> 平台分布
                </CardTitle>
              </CardHeader>
              <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="flex items-center gap-2">
                <div className="flex items-center space-x-2 mr-2">
                  <input
                    ...
                    checked={deduplicateAuthors}
                    onChange={(e) => setDeduplicateAuthors(e.target.checked)}
                  />
                  <label
                    htmlFor="content_dedup"
                ...
                >
                  <RefreshCw
                    className={cn("h-4 w-4", contentsLoading && "animate-spin")}
                  />
                </Button>
              </div>
            </CardHeader>
            <CardContent>
              <ContentDataTable
                data={contents.map((item) => {
                  const imageList =
                    item.media_urls || (item.cover_url ? [item.cover_url] : []);
                  ...
                  return {
                    ...
                    content: {
                      ...
                      url: item.url,
                      tags: item.source_keyword ? [item.source_keyword] : [],
                    },
                    media: {
                      cover:
                        item.cover_url ||
                        (validImages.length > 0 ? validImages[0] : undefined),
                    ...
                    meta: {
                      ...
                      crawl_time: item.crawl_time
                        ? new Date(item.crawl_time).toLocaleString()
                        : "-",
            ...
            <CardHeader className="border-b border-slate-800 pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-mono flex items-center gap-2">
                  <Terminal className="w-4 h-4 text-green-500" /> Live Execution
                  Logs
                </CardTitle>
                <Button
                  size="sm"
              ...
              <div className="h-[500px] overflow-y-auto p-4 font-mono text-[11px] space-y-1 bg-slate-950">
                {logs.length === 0 ? (
                  ...
                  logs.map((log, i) => (
                    <div
                      key={i}
                      className="whitespace-pre-wrap break-words border-b border-white/5 pb-1 mb-1 last:border-0 hover:bg-white/10 leading-relaxed transition-colors"
                    >
              ...
              <div className="space-y-5 max-w-3xl">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <AIKeywordSuggest
                      onSelect={(keywords) => {
                        const current = sentimentKeywordsStr
                          ? sentimentKeywordsStr + ", "
                          : "";
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, min_likes: val })
                          }
                          placeholder="不限"
                        ...
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, max_likes: val })
                          }
                          placeholder="不限"
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm"
                        />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, min_comments: val })
                          }
                          placeholder="不限"
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm"
                        />
                        ...
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, max_comments: val })
                          }
                          placeholder="不限"
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm"
                        />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, min_fans: val })
                          }
                          placeholder="最少粉丝"
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm"
                        />
                        ...
                        <CleanNumberInput
                          ...
                          onChange={(val) =>
                            setEditForm({ ...editForm, max_fans: val })
                          }
                          placeholder="最多粉丝"
                          className="w-full px-3 py-2 bg-background border border-border rounded-lg text-sm"
                        />
                ...
                <div className="border rounded-lg p-4 bg-accent/20">
                  ...
                  <div className="max-w-xl">
                    <MultiSelect
                      ...
                      onChange={(val) =>
                        setEditForm({ ...editForm, alert_channels: val })
                      }
                      placeholder="选择推送渠道..."
                    />
